    ...
    <div className="page">
      ...
      <div className="page-head">
        <div>
          ...
          <p className="page-sub">Gestión de cuentas bancarias y movimientos</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn ghost sm" onClick={() => setShowNuevaTransferencia(true)}>
            <Icon name="transfer" size={12} /> Transferencia
          </button>
      ...
      <div className="stat-grid" style={{ marginBottom: 20 }}>
        <div className="stat">
          <div className="label">Saldo total GTQ</div>
          <div className="val mono">{Qf(totalGTQ)}</div>
          <div className="delta up">
            {ACCOUNTS.filter(a => a.moneda === 'GTQ' && a.estado === 'activa').length} cuentas activas
          </div>
        </div>
        <div className="stat">
          <div className="label">Saldo total USD</div>
          <div className="val mono">{USDf(totalUSD)}</div>
          <div className="delta up">
            {ACCOUNTS.filter(a => a.moneda === 'USD' && a.estado === 'activa').length} cuentas activas
          </div>
        </div>
        <div className="stat">
          <div className="label">Diferencias contables</div>
          <div className="val mono">{diferencias}</div>
          <div className={`delta ${diferencias > 0 ? 'dn' : 'up'}`}>
            {diferencias > 0 ? 'Requieren conciliación' : 'Todo conciliado'}
          </div>
        </div>
        <div className="stat">
          <div className="label">Movimientos hoy</div>
          <div className="val mono">{movsHoy}</div>
          <div className="delta up">Último: hace 2 h</div>
        </div>
      </div>
      ...
      <div className="tabs">
        <div className={`tab ${tab === 'cuentas'        ? 'active' : ''}`} onClick={() => setTab('cuentas')}>
          Cuentas <span className="count">{ACCOUNTS.filter(a => a.estado === 'activa').length}</span>
        </div>
        <div className={`tab ${tab === 'movimientos'    ? 'active' : ''}`} onClick={() => setTab('movimientos')}>
          Movimientos <span className="count">{MOVEMENTS.length}</span>
        </div>
        <div className={`tab ${tab === 'transferencias' ? 'active' : ''}`} onClick={() => setTab('transferencias')}>
          Transferencias <span className="count">{TRANSFERS.length}</span>
        </div>
        <div className={`tab ${tab === 'bancos'         ? 'active' : ''}`} onClick={() => setTab('bancos')}>
          Bancos <span className="count">{BANKS_CAT.length}</span>
        </div>
      </div>
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                  ...
                  {a.estado === 'inactiva'
                    ? <span className="pill">Inactiva</span>
                ...
                {diff > 0 && (
                  <div className="delta dn" style={{ marginTop: 6 }}>
                    Diferencia contable: {Qf(diff)}
                  </div>
                )}
                <div className="muted" style={{ fontSize: 11, marginTop: 8 }}>Último mov.: {a.ultimoMov}</div>
              </div>
            );
          })}
        </div>
      ...
      {tab === 'transferencias' && (
        <div className="card">
          ...
          <table className="tbl">
            ...
            <tbody>
              {TRANSFERS.map(t => {
                const orig = ACCOUNTS.find(a => a.id === t.origen);
                const dest = ACCOUNTS.find(a => a.id === t.destino);
                return (
                  <tr key={t.id}>
                    ...
                    <td>{dest?.alias}</td>
                    <td>{t.concepto}</td>
                    <td className="num mono">{Qf(t.monto)}</td>
                    <td>
                      {t.estado === 'completada'
                        ? <span className="pill success"><span className="dot" />Completada</span>
                        : <span className="pill accent"><span className="dot" />Programada</span>}
                    </td>
                  </tr>
                );
              })}
        ...
        <div className="card">
          <div className="card-head">
            ...
            <button className="btn sm" onClick={() => { setFormBanco(EMPTY_BANCO); setEditBanco(false); setShowNuevoBanco(true); }}>
              <Icon name="plus" size={12} /> Agregar banco
            </button>
          </div>
          <table className="tbl">
            <thead>
              <tr><th>Código</th><th>Nombre</th><th>SWIFT / BIC</th><th>País</th><th>Estado</th><th className="num">Cuentas</th><th></th></tr>
            </thead>
            <tbody>
              {BANKS_CAT.map(b => {
        ...
        <div className="drawer-overlay" onClick={() => setDrawerCuenta(null)}>
          <div className="drawer" onClick={e => e.stopPropagation()}>
            <div className="drawer-head">
              ...
              <button className="icon-btn" onClick={() => setDrawerCuenta(null)}><Icon name="close" /></button>
            </div>
            <div className="drawer-body">
              <div className="stat" style={{ marginBottom: 20 }}>
                ...
                {drawerCuenta.saldo !== drawerCuenta.saldoContable && (
                  <div className="delta dn">Saldo contable: {Qf(drawerCuenta.saldoContable)}</div>
                )}
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '6px 16px', fontSize: 12.5, marginBottom: 20 }}>
                <div className="muted">N.° de cuenta</div>  <div className="code">{drawerCuenta.numero}</div>
                <div className="muted">Tipo</div>            <div style={{ textTransform: 'capitalize' }}>{drawerCuenta.tipo}</div>
                <div className="muted">Moneda</div>          <div>{drawerCuenta.moneda}</div>
                <div className="muted">Estado</div>          <div style={{ textTransform: 'capitalize' }}>{drawerCuenta.estado}</div>
                <div className="muted">Apertura</div>        <div>{drawerCuenta.fechaApertura}</div>
                <div className="muted">Último mov.</div>     <div>{drawerCuenta.ultimoMov}</div>
              </div>

              <div style={{ fontWeight: 600, fontSize: 12, marginBottom: 8 }}>Últimos movimientos</div>
              <table className="tbl" style={{ fontSize: 11.5 }}>
                <thead><tr><th>Fecha</th><th>Descripción</th><th className="num">Monto</th></tr></thead>
                <tbody>
                  {movsDeCuenta.map(m => (
                    <tr key={m.id}>
                      <td className="muted">{m.fecha}</td>
                  ...
                  {movsDeCuenta.length === 0 && (
                    <tr><td colSpan={3} className="muted" style={{ textAlign: 'center', padding: '12px 0' }}>Sin movimientos</td></tr>
                  )}
                </tbody>
              </table>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 20 }}>
                ...
                >
                  <Icon name="transfer" size={12} /> Transferir
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="modal-overlay" onClick={() => setShowNuevaCuenta(false)}>
          <div className="modal" style={{ width: 480 }} onClick={e => e.stopPropagation()}>
            <div className="modal-head">
              ...
              <button className="icon-btn" onClick={() => setShowNuevaCuenta(false)}><Icon name="close" /></button>
            </div>
            <div className="modal-body">
              <div className="form-grid">
                <div className="field span-2">
                  ...
                  <input
                    placeholder="Ej. Cuenta Principal GTQ"
            ...
            <div className="modal-foot">
              <button className="btn ghost" onClick={() => setShowNuevaCuenta(false)}>Cancelar</button>
              <button
                className="btn"
                disabled={!formCuenta.alias || !formCuenta.banco || !formCuenta.numero}
                onClick={() => { pushToast('Cuenta creada correctamente', 'success'); setShowNuevaCuenta(false); }}
              >
                Crear cuenta
              </button>
            </div>
        ...
        <div className="modal-overlay" onClick={() => setShowNuevaTransferencia(false)}>
          <div className="modal" style={{ width: 440 }} onClick={e => e.stopPropagation()}>
            <div className="modal-head">
              ...
              <button className="icon-btn" onClick={() => setShowNuevaTransferencia(false)}><Icon name="close" /></button>
            </div>
            <div className="modal-body">
              <div className="form-grid">
                <div className="field span-2">
                ...
                <div className="field">
                  ...
                  {formTrf.origen && (
                    <div className="muted" style={{ fontSize: 11 }}>
                      Disponible: {fmt(ACCOUNTS.find(a => a.id === formTrf.origen))}
                    </div>
                  )}
                </div>
                <div className="field">
            ...
            <div className="modal-foot">
              <button className="btn ghost" onClick={() => setShowNuevaTransferencia(false)}>Cancelar</button>
              <button
                className="btn"
                disabled={!canTransfer}
                onClick={() => { pushToast('Transferencia registrada', 'success'); setShowNuevaTransferencia(false); }}
              >
                Registrar
              </button>
            </div>
        ...
        <div className="drawer-overlay" onClick={() => setDrawerBanco(null)}>
          <div className="drawer" onClick={e => e.stopPropagation()}>
            <div className="drawer-head">
              ...
              <button className="icon-btn" onClick={() => setDrawerBanco(null)}><Icon name="close" /></button>
            </div>
            <div className="drawer-body">
              <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '6px 16px', fontSize: 12.5, marginBottom: 24 }}>
                <div className="muted">Código</div>       <div className="code">{drawerBanco.id}</div>
                <div className="muted">SWIFT / BIC</div>  <div className="code">{drawerBanco.swift}</div>
                <div className="muted">País</div>          <div>{drawerBanco.pais}</div>
                ...
                <div className="muted">Cuentas vinculadas</div>
                <div>{ACCOUNTS.filter(a => a.banco === drawerBanco.id).length}</div>
              </div>

              {ACCOUNTS.filter(a => a.banco === drawerBanco.id).length > 0 && (
                <>
                  <div style={{ fontWeight: 600, fontSize: 12, marginBottom: 8 }}>Cuentas vinculadas</div>
                  <table className="tbl" style={{ fontSize: 11.5, marginBottom: 24 }}>
                    <thead><tr><th>Alias</th><th>Tipo</th><th className="num">Saldo</th></tr></thead>
                    <tbody>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                >
                  <Icon name="edit" size={12} /> Editar información
                </button>
                <button
                  className="btn ghost sm"
                  onClick={() => { pushToast(`Banco ${drawerBanco.activo ? 'desactivado' : 'activado'}`, 'success'); setDrawerBanco(null); }}
                >
                  <Icon name={drawerBanco.activo ? 'close' : 'check'} size={12} />
                  {drawerBanco.activo ? 'Desactivar banco' : 'Activar banco'}
                </button>
                <button
                  className="btn ghost sm"
                  style={{ color: 'var(--danger)', borderColor: 'var(--danger)' }}
                  disabled={ACCOUNTS.filter(a => a.banco === drawerBanco.id).length > 0}
                  title={ACCOUNTS.filter(a => a.banco === drawerBanco.id).length > 0 ? 'No se puede eliminar: tiene cuentas vinculadas' : ''}
                  onClick={() => { pushToast('Banco eliminado', 'danger'); setDrawerBanco(null); }}
                >
                  <Icon name="alert" size={12} /> Eliminar banco
                </button>
                {ACCOUNTS.filter(a => a.banco === drawerBanco.id).length > 0 && (
                  <div className="muted" style={{ fontSize: 11 }}>
                    No se puede eliminar: tiene {ACCOUNTS.filter(a => a.banco === drawerBanco.id).length} cuenta(s) vinculada(s).
                  </div>
                )}
              </div>
            </div>
      ...
      {showNuevoBanco && (
        <div className="modal-overlay" onClick={() => setShowNuevoBanco(false)}>
          <div className="modal" style={{ width: 440 }} onClick={e => e.stopPropagation()}>
            <div className="modal-head">
              <span>{editBanco ? `Editar banco · ${formBanco.id}` : 'Agregar banco'}</span>
              <button className="icon-btn" onClick={() => setShowNuevoBanco(false)}><Icon name="close" /></button>
            </div>
            <div className="modal-body">
              <div className="form-grid">
                <div className="field span-2">
            ...
            <div className="modal-foot">
              <button className="btn ghost" onClick={() => setShowNuevoBanco(false)}>Cancelar</button>
              <button
                className="btn"
                disabled={!formBanco.nombre || !formBanco.id}
                onClick={() => { pushToast(editBanco ? 'Banco actualizado' : 'Banco agregado correctamente', 'success'); setShowNuevoBanco(false); }}
              >
                {editBanco ? 'Guardar cambios' : 'Agregar'}
              </button>
            </div>
          </div>
        </div>
